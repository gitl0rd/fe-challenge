...
const Page = (props) => {
    ...
    const [sansList, setSansList] = useState([]);
    const [monoList, setMonoList] = useState([]);

    useEffect(() => {
        //use Font API to get lists
        fetch('https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyCNxgAaVNEzCPTCWaRpx3wj72oSc65WuAs')
        .then(
            (response) => {
              ...
              //success, fill list with possible font names
              response.json().then((data) => {
                //console.log(data);
                setSerifList(data.items.filter((v) => { return v.category == 'serif'}).map((v) => { return v.family}));
                setSansList(data.items.filter((v) => { return v.category == 'sans-serif'}).map((v) => { return v.family}));
                setMonoList(data.items.filter((v) => { return v.category == 'monospace'}).map((v) => { return v.family}));
                return;
              });
            }
          );
    ...
    const changeFont = (type) => {
        ...
        if (type == 'all' || type == 'sans') {
            r = Math.floor((Math.random() * sansList.length));
            fam = sansList[r];
            //console.log(fam);
        ...
        if (type == 'all' || type == 'serif') {
            r = Math.floor((Math.random() * serifList.length));
            fam = serifList[r];
            console.log(fam);
            WebFont.load({
                google: {
        ...
        if (type == 'all' || type == 'mono') {
            r = Math.floor((Math.random() * monoList.length));
            fam = monoList[r]
            console.log(fam);
            WebFont.load({
                google: {
        ...
        //do this stuff once instead of thrice? not very sexy
    };

    return (
        <div className={classes.Main}>
            <ButtonHouse title={props.title} click={changeFont} serif={serif}/>
            <div>
                <h1 style={{fontFamily: serif}}>{props.title}</h1>
                <Section sans={sans} mono={mono}>
                    <p><strong>Waggle dance</strong> is a term used in beekeeping and ethology for a particular figure-eight dance of the honey bee. By performing this dance, successful foragers can share information about the direction and distance to patches of flowers yielding nectar and pollen, to water sources, or to new nest-site locations with other members of the colony.</p>
                ...
                {/* <TOC/> */}
                <Section heading={'Description'} sans={sans} mono={mono}>
                ...
                <Section heading={'Mechanism'} sans={sans} mono={mono}>
                ...
                <Section heading={'Controversy'} sans={sans} mono={mono}>
                ...
                <Section heading={'Evolution'} sans={sans} mono={mono}>
                ...
                <Section heading={'Applications to operations research'} sans={sans} mono={mono}>
