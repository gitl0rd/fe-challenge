...
const Page = (props) => {
    ...
    const [sans, setSans] = useState('sans-serif');
    const [mono, setMono] = useState('monospace');
        ...
        .then(
            (response) => {
              ...
              response.json().then((data) => {
                //console.log(data);
                serifList = data.items.filter((v) => { return v.category == 'serif'}).map((v) => { return v.family.replace(/ /g, '+')});
                sansList = data.items.filter((v) => { return v.category == 'sans-serif'}).map((v) => { return v.family.replace(/ /g, '+')});
                monoList = data.items.filter((v) => { return v.category == 'monospace'}).map((v) => { return v.family.replace(/ /g, '+')});
                return;
              });
            }
          );
    ...
    const changeFont = (type) => {
        //console.log(type);
        //get random index fro relevant font array and useState
    };

    return (
        <div className={classes.Main}>
            <ButtonHouse title={props.title} click={changeFont}/>
            <div>
                <h1>{props.title}</h1>
                <Section>
                ...
                <Section heading={'Applications to operations research'}>
...
//NEED TO FIND OUT HOW TO PASS AROUND FONTS TO SECTIONS???????